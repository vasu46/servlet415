	...
	public void doPost(HttpServletRequest req,HttpServletResponse res)throws ServletException,IOException{
		...
		//set Content type in
         ...
         String tage=req.getParameter("page");
			  ...
			  //server side Form validatio List<String> errorList=new ArrayList();
			  if(name==null || name.equals("")||name.length()==0)
			  errorList.add("Person name must be required"); else if(name.length()<5 &&
			  name.length()>10)
			  errorList.add("Person name must be Abov 5 ChARS and lessthan 10"); else
			 ...
			 // validation error messaage 
			 if(errorList.size()>0) { 
				 for(String msg:errorList){ 
					 pw.println("<li style='color:red'><b>"+msg+"</li></b>"); }//for return;
			  }//if
